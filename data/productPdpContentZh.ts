...
export function buildMayfairKaftanContentZh(color?: string): ProductPdpContent {
  ...
  return {
    introParagraphs: [
      `Mayfair Kaftan 为懂得优雅从不静止的女性而设计。以 ${adj} 绉纱雪纺裁制，并搭配一体式内搭连衣裙，这款雪纺 kaftan 从肩部到下摆自然垂坠，呈现流动廓形。`,
      '左肩垂落的柔和围巾细节可借助 Bint Saeed 标志性金色徽章别针斜向搭配于身前。隐藏式内侧系带可实现多种廓形变化，既可呈现披风般的流动感，也可形成更清晰的线条。由此，这件单品会随着穿着者而变化，自然适配不同场合与时刻。',
...
export function buildNothingHillKaftanContentZh(color?: string): ProductPdpContent {
  ...
  return {
    introParagraphs: [
      ...
      '轻盈且灵动的雪纺自肩线自然垂落至下摆，在保持均衡轮廓的同时带来动态感。Bint Saeed 标志性金色徽章低调点缀于前身，传递品牌识别。',
...
export function buildBelgraviaAbayaContentZh(color?: string): ProductPdpContent {
  ...
  return {
    introParagraphs: [
      ...
      '提供 Deep Black 与 Navy Blue 两种配色。这款 abaya 的标志性细节是手工编织饰边，灵感来自 Al Khous——代代相传的阿联酋传统棕榈叶编织工艺。其纹样呼应棕榈叶编织几何，为克制优雅的廓形注入纹理与文化工艺感。',
      'Belgravia Abaya 于阿布扎比打造，体现 Bint Saeed 以当代设计延续传统工艺的承诺。宽松的 Bisht 风格剪裁带来优雅动态并保持精致结构，隐藏口袋与全里衬结构进一步提升舒适与穿着便利。',
      'Belgravia Abaya 为跨场景、跨地域与跨生活方式而设计。可用于利雅得婚礼、伦敦晚宴、巴黎活动，也可融入海湾地区日常生活。它追求恒久而非潮流，面向重视优雅、工艺与长期价值的女性。',
      '与所有 Bint Saeed abaya 一样，Belgravia Abaya 为按单制作，并可在隐藏口袋内定制姓名、日期或有意义的信息。',
    ],
    productDetails: [
      'Bisht 灵感 abaya 廓形',
...
export function buildKensingtonAbayaContentZh(): ProductPdpContent {
  return {
    introParagraphs: [
      ...
      '全里衬采用柔软绉料，并配有两个隐藏侧袋，Kensington Abaya 在保持干净优雅廓形的同时兼顾实用与精致。与所有 Bint Saeed abaya 一样，可通过隐藏式内侧标签进行个性化定制，加入姓名、日期或有意义的信息，尤其适合作为礼赠。',
    ...
    productDetails: [
      ...
      '轻薄肩垫',
      '前中按扣闭合',
      'Bint Saeed 标志性织饰，灵感源自传统 Al Khous 棕榈叶编织',
      '两个隐藏侧袋',
      '柔软绉料里衬',
      '可选隐藏式内侧个性化标签',
      '衣长：138 厘米 / 54.5 英寸',
...
export function buildKnightsbridgeAbayaJacketContentZh(color?: string): ProductPdpContent {
  ...
  return {
    introParagraphs: [
      ...
      'Khous Jacket Abaya 为自信行走于人生、坚持自我节奏的女性而生。它介于 abaya 与夹克之间，将传统着装的从容与当代外套的力量感结合。',
      ...
      '以 Bint Saeed 标志性金色 Knotted Lines of Lineage 纽扣收尾，承载品牌长期延续的核心设计语言之一。它灵感源于连接代际的纽带，提醒我们：最有意义的事物，往往是那些被我们持续传承的部分。',
      'Khous Jacket Abaya 于阿布扎比打造，体现 Bint Saeed 将阿联酋文化遗产元素带入当代衣橱的承诺。无论是伦敦咖啡时刻、旅行日程、迪拜会面，还是海湾地区日常生活，它都为理解“风格不止属于特别场合”的女性提供鲜明轮廓。',
      '舒适、百搭且为高频穿着而设计，Khous Jacket Abaya 诠释了真正优雅不仅在重要时刻显现，更体现在女性每天如何选择呈现自己。',
    ],
    productDetails: [
      `${colorLabel} 色夹克式 abaya，宽松廓形`,
      '尖领设计',
      '隐藏式前门襟纽扣闭合',
      '两个胸前口袋',
      '两个隐藏侧袋',
      '肩部袢带细节',
      '长袖配纽扣袖口',
      '胸袋与袖口处饰有 Bint Saeed 标志性 Khous 灵感织饰',
      'Bint Saeed 标志性金色 Knotted Lines of Lineage 纽扣',
      '一体式内搭连衣裙',
    ...
    careDetails: ['仅限专业干洗'],
    stylePairingNote: getKnightsbridgeStylePairingNote('knightsbridge-abaya-jacket', catalogColor, 'zh'),
