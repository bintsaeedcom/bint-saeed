#!/usr/bin/env python3
"""One-off generator for lib/products/optimisedKeywordsI18n.ts"""

HEADER = '''import type { AppLocale } from '@/lib/i18n/routing'
...
# Pool translations: [en, ar, fr, it, es, ru, zh, de, nl, pt]
GCC_UAE_ROWS = [
...
def format_rows(name: str, rows: list) -> str:
    ...
    for row in rows:
        parts = ', '.join(f"'{esc(c)}'" for c in row)
...
def main() -> None:
    out = HEADER
    out += format_rows('GCC_UAE_ROWS', GCC_UAE_ROWS) + '\n\n'
    out += format_rows('EUROPEAN_ROWS', EUROPEAN_ROWS) + '\n\n'
    out += format_rows('BRAND_GEO_ROWS', BRAND_GEO_ROWS) + '\n\n'
    out += format_rows('OCCASION_ROWS', OCCASION_ROWS) + '\n\n'
    out += format_rows('DISCOVERY_ROWS', DISCOVERY_ROWS) + '\n\n'
    out += format_rows('NOTHING_HILL_ROWS', NOTHING_HILL_ROWS) + '\n\n'
    out += format_rows('MAYFAIR_ROWS', MAYFAIR_ROWS) + '\n'
    out += FOOTER
    path = '/Users/sunain/Desktop/bint saeed/lib/products/optimisedKeywordsI18n.ts'
    with open(path, 'w', encoding='utf-8') as f:
        f.write(out)
    print(f'Wrote {path}')
    print(f'discovery rows: {len(DISCOVERY_ROWS)}')
