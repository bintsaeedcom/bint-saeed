#!/usr/bin/env python3
"""Generate lib/products/abayaSchemaKeywordsI18n.ts and belgraviaSchemaKeywordsI18n.ts"""

from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]

ABAYA_HEADER = '''import type { AppLocale } from '@/lib/i18n/routing'
...
BELGRAVIA_HEADER = '''import type { AppLocale } from '@/lib/i18n/routing'
import { BRAND_NAME } from '@/lib/i18n/brandProperNouns'
...
def format_rows(name: str, rows: list) -> str:
    lines = [f'const {name}: KwRow[] = [']
    for row in rows:
        escaped = [r.replace('\\', '\\\\').replace("'", "\\'") for r in row]
        lines.append(f"  [{', '.join(repr(r) for r in escaped)}],")
    lines.append(']')
    return '\n'.join(lines)


def main() -> None:
    abaya_path = ROOT / 'lib/products/abayaSchemaKeywordsI18n.ts'
    belgravia_path = ROOT / 'lib/products/belgraviaSchemaKeywordsI18n.ts'

    abaya_content = (
        ABAYA_HEADER
        + format_rows('SHARED_ABAYA_KEYWORD_ROWS', SHARED_ABAYA_KEYWORD_ROWS)
        + ABAYA_FOOTER
    )
    belgravia_content = (
        BELGRAVIA_HEADER
        + format_rows('BELGRAVIA_EXCLUSIVE_ROWS', BELGRAVIA_EXCLUSIVE_ROWS)
        + BELGRAVIA_FOOTER
    )

    abaya_path.write_text(abaya_content, encoding='utf-8')
    belgravia_path.write_text(belgravia_content, encoding='utf-8')
    print(f'Wrote {abaya_path} ({len(SHARED_ABAYA_KEYWORD_ROWS)} rows)')
    print(f'Wrote {belgravia_path} ({len(BELGRAVIA_EXCLUSIVE_ROWS)} rows)')
