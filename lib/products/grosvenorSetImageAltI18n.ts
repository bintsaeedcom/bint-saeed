...
function frontAlts(): Record<AppLocale, string> {
  return altLoc(
    ...
    `BINT SAEED 承悦 Grosvenor 两件套${COLOR.zh}正面视图。香槟奶油色缎面协调衬衫与高腰及地半裙，袖口可更换天然石链饰（附赠玛瑙链），金色调腰饰与 Knotted Line 袖扣。阿布扎比至伦敦、巴黎、米兰、利雅得及全球。阿联酋阿布扎比制造。全球配送。`,
...
function sideAlts(): Record<AppLocale, string> {
  return altLoc(
    ...
    `Grosvenor 两件套${COLOR.zh}四分之三视图，展现缎面垂坠、高腰及地线条、金色调腰饰与 Knotted Line 袖扣；袖口可更换天然石服饰珠宝。奢华阿联酋晚宴两件套。阿布扎比制造。全球配送。`,
...
function backAlts(): Record<AppLocale, string> {
  return altLoc(
    ...
    `Grosvenor 两件套${COLOR.zh}背面视图，凸显缎面晚宴剪裁、及地长裙廓形与袖口 Knotted Line 细节。阿布扎比设计高端两件套。全球配送。`,
...
function lifestyle1Alts(): Record<AppLocale, string> {
  return altLoc(
    `Lifestyle image of the Bint Saeed Grosvenor Set in ${COLOR.en} styled for an evening occasion — satin blouse and maxi skirt with gold waist trim. Signature Onyx garment jewellery included with the set at the cuffs. Emirati luxury formal set from Abu Dhabi. Worldwide shipping.`,
    ...
    `Grosvenor 两件套${COLOR.zh}生活方式图，晚宴造型；袖口标志性玛瑙服饰珠宝随套装附赠。`,
...
function lifestyle2Alts(): Record<AppLocale, string> {
  return altLoc(
    ...
    `Grosvenor 两件套${COLOR.zh}第二张生活方式图，展现裙摆缎面动感、协调晚宴比例与 Knotted Line 袖扣。`,
...
function jewelleryDetailAlts(): Record<AppLocale, string> {
  return altLoc(
    `Detail image of signature Onyx garment jewellery included with the Bint Saeed Grosvenor Two-Piece Set in ${COLOR.en} — interchangeable natural-stone strands at the shirt cuffs. Bint Saeed formal evening set from Abu Dhabi.`,
    ...
    `Grosvenor 两件套${COLOR.zh}附赠玛瑙服饰珠宝细节图——衬衫袖口可更换的天然石链饰。`,
