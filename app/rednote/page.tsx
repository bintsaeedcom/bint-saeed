...
export async function generateMetadata(): Promise<Metadata> {
  return buildChinaSocialPageMetadata('rednote', await getServerLocale())
}

export default async function RedNotePage() {
  const locale = await getServerLocale()
  const jsonLd = buildChinaSocialWebPageJsonLd('rednote', locale)
    ...
    <main className={`${EDITORIAL_PAGE_SHELL} bg-brand-pageCanvas text-neutral-900`}>
      ...
      <div className={`${EDITORIAL_PAGE_CONTAINER} ${SITE_CONTENT_TOP_PAD} pb-20 md:pb-28`}>
        ...
        <p className="mt-6 max-w-2xl font-montserrat text-sm leading-relaxed tracking-wide text-neutral-600 md:text-base">
          Official RedNote (Xiaohongshu) account for the Abu Dhabi fashion house. In Chinese, the
          house name is 承悦 — the only language in which Bint Saeed uses a distinct brand name.
        </p>

        <dl className="mt-12 max-w-xl space-y-6 font-montserrat text-sm">
          <div>
            <dt className="text-[10px] uppercase tracking-[0.22em] text-brand-dustyBlue">Red ID</dt>
            <dd className="mt-2 text-brand-darkRed">{REDNOTE_ID}</dd>
          </div>
          <div>
        ...
        <p className="mt-10 max-w-2xl font-montserrat text-sm leading-relaxed text-neutral-600">
          Open the RedNote / 小红书 app and search for the Red ID above, or for{' '}
        ...
        {REDNOTE_PROFILE_URL ? (
          <p className="mt-8">
            <a
              href={REDNOTE_PROFILE_URL}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex font-montserrat text-[11px] uppercase tracking-[0.22em] text-brand-darkRed underline decoration-brand-darkRed/25 underline-offset-8"
            >
              Open RedNote profile
            </a>
          </p>
        ) : null}
